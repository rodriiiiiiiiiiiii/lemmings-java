...
public class Trampolin extends GameObject {

    public Trampolin(GameWorld game, Position pos) {
        super(game, pos);
    }

    public Trampolin() { /* para la factoría */ }

    @Override
    public String toString() {
        return "(" + pos.getCol() + "," + pos.getRow() + ") Trampolin";
    }
    ...
    public GameObject parse(String line, GameWorld game)
        ...
        if (!name.equalsIgnoreCase("T") && !name.equalsIgnoreCase("Trampolin"))
            return null;
        if (w.length != 2)
            throw new tp1.exceptions.ObjectParseException("Incorrect parameter count for Trampolin");
        Position p = getPositionFrom(line, game);
        game.checkPositionInBounds(p);
        return new Trampolin(game, p);
    }
